...
export default class RichToolbar extends Component {
  ...
  _renderAction(action, selected, callback) {
    var selectedItems = this.state.items
    return this.props.renderCustomAction
      ? this.props.renderCustomAction({action, selected, callback, selectedItems})
    ...
    return (
      <View style={vStyle}>
        <FlatList
          ...
          renderItem={({item}) => this._renderAction(item.action, item.selected, this._onPress.bind(this))}
